                ...
                for epoch in range(self.epoch + 1, max_epochs + 1):
                    ...
                    if self.settings.use_swa:
                        if swa_init and epoch == self.settings.swa_epoch:
                            self.net_swa = torch.optim.swa_utils.AveragedModel(self.actor.net)
                            swa_init = False
                    ...
                    if epoch > (max_epochs - 1) or save_every_epoch or epoch % self.settings.save_interval == 0 \
                            or epoch % 40 == 0 or epoch in save_epochs or epoch > (max_epochs - 5):
